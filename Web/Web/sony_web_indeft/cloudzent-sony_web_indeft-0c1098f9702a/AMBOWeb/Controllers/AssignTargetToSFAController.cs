...
{
    public class AssignTargetToSFAController : SonyBaseController
    {
        ...
        public AssignTargetToSFAController(IAPIMappingData IAPIMappingData, IAPICommon IAPICommon, IAPIGridData IAPIGridData)
        {
        ...
        {
            AssignTargetToSFAGet InputParam = new AssignTargetToSFAGet();
        ...
        {
            AssignTargetToSFAGet Input = new AssignTargetToSFAGet();
            Input.UserId = objUserSession.UserId;
            Input.TargetDate = TargetDate;
            if (BranchId != 0)
                Input.BranchId = BranchId;
            List<AssignTargetToSFAGrid> targetList = new List<AssignTargetToSFAGrid>();
            
            targetList = _APIGridData.ShowTargetToSFAByMonth(Input);
        ...
        public ActionResult Download(string TargetDate, Int64 BranchId)
        {
            ...
            dt.Columns.AddRange(new DataColumn[15] { new DataColumn("Month"),
            ...
            List<AssignTargetToSFAGrid> targetList = new List<AssignTargetToSFAGrid>();
            targetList = _APIGridData.GetTargetToSFAByMonth(Input);

            if (targetList != null && targetList.Count > 0)
            {
                foreach (var record in targetList)
                {
                    dt.Rows.Add(TargetDate, record.Branch, record.City, record.Location, record.SFACode, record.SFAName,
                        record.DealerCode, record.MasterCode, record.DealerName, record.SFACategory, record.IncentiveCategory,
                ...
                using (XLWorkbook wb = new XLWorkbook())
                {
                    ...
                    {
                        wb.SaveAs(stream);
                        return File(stream.ToArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "TargetToSFA.xlsx");
                    }
                }
            }
            else
        ...
        [HttpPost]
        [SessionAuthorize(ModuleId = (Int64)Modules.Incentive, SubModuleId = (Int64)SubModules.AssignTargettoSFA, Rights = (int)Right.Create)]
        public ActionResult UploadData(HttpPostedFileBase file, AssignTargetToSFAUpload Input)
        {
            ...
            AssignTargetToSFAUpload InputParam = new AssignTargetToSFAUpload();
            InputParam.UserId = objUserSession.UserId;

            InputParam.TargetDate = Input.TargetDate;

            Envelope<List<AssignTargetToSFAUploadOutput>> output = new Envelope<List<AssignTargetToSFAUploadOutput>>();
            List<AssignTargetToSFAUploadOutput> _AssignTargetToSFAUploadOutput = new List<AssignTargetToSFAUploadOutput>();
            ...
            if (file != null || InputParam.TargetDate != "")
            {
                ...
                if (dt != null)
                {
                    ...
                    var exceloutput = _APIMappingData.UploadTargetToSFAByMonth(InputParam);
                    ...
                    if (exceloutput.Data != null && exceloutput.MessageCode == (int)Acceptable.Created)
                    {
                        ...
                        _AssignTargetToSFAUploadOutput = ConvertTo<AssignTargetToSFAUploadOutput>(exceloutput.Data);
                    ...
                    else if (output.MessageCode == (int)NotAcceptable.NotModified)
                    {
                        ...
                        inp.ErrorSource = "AssignTargetToSFAExcelUpload";
                        _Common.CreateErrorLogWeb(inp);
                        output.Data = new List<AssignTargetToSFAUploadOutput>();
                        output.MessageCode = exceloutput.MessageCode;
                        output.Message = ViewBag.Message = exceloutput.Message;
                    }
                }
                ...
                if (!string.IsNullOrEmpty(fileName))
                {
                    ...
                    if (Extension == ".xls" || Extension == ".xlsx")
                    {
                        ...
                        using (DataTable dtSheet = oledbConn.GetOleDbSchemaTable(OleDbSchemaGuid.Tables, null))
                        {
                            ...
                            {
                                if ((dt.Columns[0].ColumnName != "Month") ||
                                        ...
                                        (dt.Columns[10].ColumnName != "IncentiveCategory") ||
